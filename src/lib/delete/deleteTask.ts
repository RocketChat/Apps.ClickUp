...
export async function deleteTask({ context, data, room, read, persistence, modify, http }: { context: UIKitBlockInteractionContext; data: IUIKitBaseIncomingInteraction; room: IRoom; read: IRead; persistence: IPersistence; modify: IModify; http: IHttp }) {
  ...
  if (response.statusCode == HttpStatusCode.NO_CONTENT) {
    const textSender = await modify.getCreator().startMessage().setText(`✅️ Task deleted successfully!`);
    if (room) {
