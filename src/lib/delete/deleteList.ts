...
export async function deleteList({ context, data, room, read, persistence, modify, http }: { context: UIKitBlockInteractionContext; data: IUIKitBaseIncomingInteraction; room: IRoom; read: IRead; persistence: IPersistence; modify: IModify; http: IHttp }) {
  ...
  if (response.statusCode == HttpStatusCode.OK) {
    const textSender = await modify.getCreator().startMessage().setText(`✅️ List deleted successfully!`);
    if (room) {
