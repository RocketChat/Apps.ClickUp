...
export class NotificationsController {
  ...
  constructor(read: IRead, persistence: IPersistence, user: IUser) {
    ...
    this.association = new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, `clickup-notifications`);

    this.userAssociation = new RocketChatAssociationRecord(RocketChatAssociationModel.USER, user.id);
  }

  public async getNotificationsStatus(): Promise<INotificationsStatus> {
    const [record] = await this.read.getPersistenceReader().readByAssociations([this.association, this.userAssociation]);
