...
export async function persistWorkspace({ context, data, room, read, persistence, modify, http }: { context: UIKitViewSubmitInteractionContext; data: IUIKitViewSubmitIncomingInteraction; room: IRoom; read: IRead; persistence: IPersistence; modify: IModify; http: IHttp }) {
  ...
  try {
    ...
    const textSender = await modify.getCreator().startMessage().setText(`✅️ Workspace and members data saved successfully!`);
    if (room) {
      textSender.setRoom(room);
    }
    await modify.getCreator().finish(textSender);
  } catch (error) {
    const textSender = await modify.getCreator().startMessage().setText(`❗️ Unable to save! \n Error ${error}`);
    if (room) {
      textSender.setRoom(room);
    }
    await modify.getCreator().finish(textSender);
