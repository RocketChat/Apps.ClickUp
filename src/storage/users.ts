...
export async function get_clickup_uid(read: IRead, user: string): Promise<string | undefined> {
  ...
  const associations = [new RocketChatAssociationRecord(RocketChatAssociationModel.USER, rcuid.id), new RocketChatAssociationRecord(RocketChatAssociationModel.MISC, `clickup_uid`)];
